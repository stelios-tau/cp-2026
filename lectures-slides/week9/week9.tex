...
\begin{frame}[fragile]
  ...
  \begin{itemize}
  \item[\faCode]<1-> CookingRace: ``Two-chefs'' with a shared pan.
  \item[\faCode]<1-> CheckThenWrite: Race condition.
  \item[\faCode]<1-> CheckThenWithdraw: Race condition involving withdrawals.
  \item[\faCode]<1-> Counter.java: Example from Week 7 on a shared counter.
  \item[\faCode]<1-> CounterPlus.java: Example from Week 7 on a shared counter.
  \item[\faCode]<1-> GasTankRace: Gas storage example with an interesting race
    condition.
  \end{itemize}
\end{frame}
