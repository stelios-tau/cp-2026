...
\begin{frame}[fragile]
  ...
  \item[\faBook]<2-> The goal of Project Loom is to bring similar lightweight
...
\begin{frame}[fragile]
  ...
  \item[\faBook]<2-> \textbf{Many-to-Few Mapping:}  The JVM can schedule many
  ...
  \item[\faBook]<3-> \textbf{Same API as Threads:} A virtual thread behaves like
  ...
  \item[\faBook]<4-> \textbf{Use Case:} Designed for tasks that spend a lot of
...
\begin{frame}[fragile]
  ...
  \begin{itemize}
  \item[\faBook] \textbf{Use synchronous, blocking methods:} The whole point of
  ...
  \item[\faBook] \textbf{No need for pools:} As explained earlier.
  \item[\faBook] \textbf{\texttt{ThreadLocal} and inheritance:} Be mindful ofn
...
\begin{frame}[fragile]
  ...
  \begin{itemize}
  \item[\faBook] \textbf{Thread pinning:} The whole point of
    virtual threads is to enable simple, synchronous, sequential thread-per-task
    style code. Avoid asynchronous code with callbacks.
  \item[\faBook] \textbf{CPU-heavy tasks:} As explained earlier.
  \item[\faBook] \textbf{:} Be mindful ofn
    \texttt{ThreadLocal} variables, as they can quickly eat up memory in the
    presence of a high number of virtual threads.
  \end{itemize}
\end{frame}
