...
\subtitle{Week 18 (Lecture 10) : \textbf{Virtual threads}}
...
\begin{frame}[fragile]
  ...
  \item[\faBook]<3-> \textbf{Limited Concurrency:} As such, only a limited number of
    platform threads can run (bottleneck reached before CPU/network limit).
...
\begin{frame}[fragile]
  ...
  \begin{itemize}
  \item[\faBook]<2-> \textbf{Callback Hell :}
    This leads to complex callback chains, which are hard to read and maintain. The
    logical flow of the program gets scattered across callbacks.
  \item[\faBook]<3-> \textbf{Debugging difficulty:} Asynchronous code does not
    follow normal call stack, making debugging harder.
  \item[\faBook]<4-> \textbf{Cognitive load:} Developers must rewrite simple
...
\begin{frame}[fragile, shrink=5]
  ...
  \item[\faBook]<1-> \textbf{Many-to-Many\,\footnote{More like
        Many-to-\emph{Few}.} Mapping:}  The JVM can schedule many
...
\begin{frame}[fragile]
  ...
  \begin{center}
    \includegraphics[width=11cm,keepaspectratio]{../media/plat-threads.jpg}
  \end{center}
\end{frame}
...
\begin{frame}[fragile]
  ...
  \begin{center}
    \includegraphics[width=11cm,keepaspectratio]{../media/virt-threads.jpg}
  \end{center}
\end{frame}
...
\begin{frame}[fragile]
  ...
  \item[\faTimes]<3-> \textbf{Avoid \texttt{synchronized} blocks:} If a virtual
    ...
    \texttt{ReentrantLock} instead. \textcolor{blue}{\emph{This limitation was removed in
        Java 24 (see JEP 491)!}}
  \item[\faTimes]<4-> \textbf{\texttt{ThreadLocal} and inheritance:} Be mindful of
...
\begin{frame}[fragile]
  ...
  \begin{block}{\faLightbulb \quad Virtual Threads}
    Virtual threads are a new type of Java \texttt{Thread}s that are lighter to
    create and schedule and are excellent for concurrency-heavy scenarios where
    each thread is on waiting mode (not synchronizing). They provide potentially
    much better throughput than standard threads.
  \end{block}
  \vspace{0.4cm}
  \pause
  \begin{block}{\faLightbulb \quad Structured Concurrency}
    The other part of Project Loom, \textbf{Structured Concurrency} is currently
    in preview. Much like structured programming saved us from GOTO's,
    structured concurrency aims to bring more order to concurrent programming.
    % Feel free to explore structured concurrency online, e.g.
    % \href{https://openjdk.org/jeps/480}{https://openjdk.org/jeps/480} or the
    % official tutorial.
  \end{block}
...
\begin{frame}[fragile]
  \frametitle{Key idea of Structured Concurrency}

  \begin{itemize}
  \item[\faBook]<1-> It is often that a program can be broken down in
    a set of concurrent \emph{tasks}.
  \item[\faBook]<1-> What happens when a task also consists of smaller
    concurrent tasks?
  \end{itemize}
  \vspace{0.8cm}
  \begin{block}<2->{\faLightbulb \quad Scope}
    In structured concurrency, each task is assigned its own \emph{scope}, under
    which it can spawn its own \emph{subtasks}. Think blocks in structured
    programming!
  \end{block}
\end{frame}

\begin{frame}[fragile]
  \frametitle{Tree-like structure of tasks}
  \begin{tikzpicture}[
  task/.style={
  ...
  root/.style={task, draw=blue!60, fill=blue!8, font=\bfseries},
  green/.style={task, draw=green!50!black, fill=green!8},
  orange/.style={task, draw=orange!80!black, fill=orange!10},
  purple/.style={task, draw=purple!70!black, fill=purple!8},
  edge from parent/.style={draw, thick},
  level 1/.style={sibling distance=5cm, level distance=1.8cm},
  level 2/.style={sibling distance=2cm, level distance=1.7cm},
  level 3/.style={sibling distance=1.8cm, level distance=1.6cm}
]

\node[root] {Main Task\\ \normalfont(root scope)}
  child { node[green] {Subtask A\\ \normalfont(child scope)}
    child { node[green] {A1\\ \normalfont(subtask)} }
    child { node[green] {A2\\ \normalfont(subtask)} }
  }
  child { node[orange] {Subtask B\\ \normalfont(child scope)}
    child { node[orange] {B1\\ \normalfont(subtask)} }
    child { node[orange] {B2\\ \normalfont(subtask)}
      child { node[orange] {B2.1\\ \normalfont(subtask)} }
      child { node[orange] {B2.2\\ \normalfont(subtask)} }
    }
  }
  child { node[purple] {Subtask C\\ \normalfont(child scope)}
    child { node[purple] {C1\\ \normalfont(subtask)} }
    child { node[purple] {C2\\ \normalfont(subtask)} }
    child { node[purple] {C3\\ \normalfont(subtask)} }
  };
...
\begin{frame}[fragile]
  \frametitle{Unstructured concurrency flattens trees}
  \begin{tikzpicture}[
  task/.style={
    draw, rounded corners,
    minimum width=1cm,
...
\begin{frame}[fragile]
  ...
  orange/.style={task, draw=orange!80!black, fill=orange!10},
  purple/.style={task, draw=purple!70!black, fill=purple!8},
  lost/.style={draw=gray!60, dashed, thick, -}
]

% Nodes in an intentionally non-hierarchical layout
\node[root]   (main) at (0,0) {Main Task};

\node[green]  (A)    at (-3.8,1.8) {Subtask A};
\node[orange] (B)    at (3.5,1.5) {Subtask B};
\node[purple] (C)    at (0,-3.0) {Subtask C};

\node[green]  (A1)   at (-5.0,-0.8) {A1};
\node[green]  (A2)   at (-2.0,-2.3) {A2};

\node[orange] (B1)   at (1.8,3.4) {B1};
\node[orange] (B2)   at (5.0,-0.6) {B2};
\node[orange] (B21)  at (2.8,-2.4) {B2.1};
\node[orange] (B22)  at (5.2,2.9) {B2.2};

\node[purple] (C1)   at (-1.0,3.2) {C1};
\node[purple] (C2)   at (-4.4,-3.0) {C2};
\node[purple] (C3)   at (0.9,-1.2) {C3};

% Optional faint/dashed "lost structure" connections
\draw[lost,visible on=<2->] (main) -- (A);
\draw[lost,visible on=<2->] (main) -- (B);
\draw[lost,visible on=<2->] (main) -- (C);

\draw[lost,visible on=<2->] (A) -- (A1);
\draw[lost,visible on=<2->] (A) -- (A2);

\draw[lost,visible on=<2->] (B) -- (B1);
\draw[lost,visible on=<2->] (B) -- (B2);
\draw[lost,visible on=<2->] (B2) -- (B21);
\draw[lost,visible on=<2->] (B2) -- (B22);

\draw[lost,visible on=<2->] (C) -- (C1);
\draw[lost,visible on=<2->] (C) -- (C2);
\draw[lost,visible on=<2->] (C) -- (C3);
\end{tikzpicture}
...
\begin{frame}[fragile]
  \frametitle{Tree-like structure of tasks}
  \begin{itemize}
  \item[\faBook]<1-> There are practical benefits in reflecting the tree-like
    structure in the code, leading to graceful management of tasks/threads.
  \item[\faBook]<1-> \textbf{If a task splits into concurrent subtasks, then
      the all subtasks return to the same ``parent'' block}.
  \end{itemize}
  \vspace{0.2cm}
  \begin{block}<2->{\faLightbulb \quad Workflow}
    \begin{enumerate}
    \item Open a new scope, like \texttt{StructuredTaskScope}.
    \item Fork subtasks.
    \item Join subtasks.
...
\end{lstlisting}
  ...
  This is the \emph{default} joiner, which cancels the scope if any subtasks
  fail and returns nothing if successful.
...
\begin{frame}[fragile]
  ...
  \begin{itemize}
  \item[\faBook]<1-> Official Java Concurrency tutorial
    (\href{https://docs.oracle.com/en/java/javase/21/core/concurrency.html}{https://docs.oracle.com/en/java/javase/21/core/concurrency.html})
  \item[\faBook]<1-> JDK Enhancement Proposal 444
    (\href{https://openjdk.org/jeps/444}{https://openjdk.org/jeps/444})
  \item[\faBook]<1-> Rock the JVM blog post:
    (\href{https://rockthejvm.com/articles/the-ultimate-guide-to-java-virtual-threads}{https://rockthejvm.com/articles/the-ultimate-guide-to-java-virtual-threads})
  \item[\faBook]<1-> Blog post by Cay Horstmann:
    (\href{https://horstmann.com/unblog/2023-06-27/index.html}{https://horstmann.com/unblog/2023-06-27/index.html})
  \item[\faBook]<1-> More on Project Loom
    (\href{https://www.baeldung.com/openjdk-project-loom}{https://www.baeldung.com/openjdk-project-loom})
    \item[\faBook]<1-> Structured Concurrency presentation (\href{https://www.youtube.com/watch?v=IosvCU9qM5M}{https://www.youtube.com/watch?v=IosvCU9qM5M})
  \end{itemize}
...
\begin{frame}[fragile]
  ...
  \begin{itemize}
  \item[\faCode]<1-> Week19/VirtualThreadSleepDemo.java : example of improved
    throughput via virtual threads on blocking tasks.
  \item[\faCode]<1-> Week19/VirtualThreadHTTPDemo.java : example of improved
    throughput via virtual threads on HTTP requests.
  \item[\faCode]<1-> Week19/VirtualThreads1.java and
    Week19/VirtualThreads2.java : two more examples of virtual threads.
  \end{itemize}
\end{frame}
