...
frame = trBL , firstnumber = last , escapeinside={(*@}{@*)}]
class SharedBuffer {
...
\section{Blocking queues}

\begin{frame}[fragile]
  \frametitle{Using queues}
\begin{lstlisting}[basicstyle=\fontsize{8}{9}\selectfont\ttfamily, language = Java ,
frame = trBL , firstnumber = last , escapeinside={(*@}{@*)}]
...
\end{lstlisting}
  \begin{block}<2->{\faLightbulb \quad Important}
    \texttt{LinkedBlockingQueue} and \texttt{ArrayBlockingQueue} is FIFO (First
    In - First Out).
  \end{block}
  \vspace{0.2cm}
  \begin{block}<3->{\faLightbulb \quad Important}
    Methods \texttt{put} and \texttt{take}. \texttt{add} and \texttt{remove}
    don't block.
  \end{block}
  \uncover<4->{
    \begin{tikzpicture}[overlay, remember picture]
