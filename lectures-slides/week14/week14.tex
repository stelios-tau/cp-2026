...
\begin{frame}[fragile]
  ...
  \item[\faBook]<1-> Presence of a data structure representing a to-do list.
    \begin{itemize}
    \item[\faBook]<1-> Producers identify work and creates \emph{tasks}.
    \item[\faBook]<1-> Consumers pick up and complete the tasks.
    \end{itemize}
...
\begin{frame}[fragile]
  ...
  \begin{itemize}
  \item[\faCookie]<1-> The kitchen has several such examples.
    \begin{itemize}
    \item[\faBook]<1-> 
    \item[\faBook]<1-> 
    \end{itemize}
  ...
  \begin{block}<1->{\faCookie \quad Kitchen}
    Several examples:
    \begin{itemize}
    \item[\faBook] Waiter brings orders, the kitchen completes orders.
    \item[\faBook] Chefs produce tasks for each dish that the cooks must
      complete.
    \end{itemize}
  \end{block}

  \begin{block}<1->{\faSearch \quad Factory}
