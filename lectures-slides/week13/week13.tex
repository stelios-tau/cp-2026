...
\subtitle{Week 13 (Lecture 7) : \textbf{Queues, tasks and executors}}
...
\begin{frame}[fragile]
  \frametitle{One important nuance}
...
% \begin{frame}[fragile]
%   \frametitle{Recall an important nuance}

%   \begin{block}<2->{\faLightbulb \quad Relative overhead}
...
class SharedBuffer {
    ...
    public synchronized String consume() throws InterruptedException {
        ...
        notify(); // Signal waiting producers
        return item;
    }
}\end{lstlisting}
\uncover<2->{
...
\begin{frame}[fragile]
  \frametitle{Stopping  in \texttt{BlockingQueue}-based producer-consumers}

  \begin{enumerate}
  \item Stop producing new tasks.
  \item Insert the so-called \emph{poison-pill} tasks in the queue.
  \item Terminate the producer.
  \end{enumerate}
  \vspace{0.6cm}
  \begin{block}<2->{\faSkullCrossbones \quad Poison-pills}
    Special tasks that signal consuming threads to return.
  \end{block}
\end{frame}
