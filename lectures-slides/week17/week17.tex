...
\subtitle{Week 17 (Lecture 9) : \textbf{Liveness hazards and performance}}
...
\begin{frame}[fragile]
  ...
  \begin{center}
    \begin{tikzpicture}[
      ...
      \draw[arrow] (A) -- (T1);
      \draw[arrow] (B) -- (T2);
      \draw[arrow,purple,dashed] (T1) -- (B);
      \draw[arrow,purple,dashed] (T2) to[bend left=20] (A);
...
\begin{frame}[fragile]
  ...
  \begin{center}
    \begin{tikzpicture}[
      ...
      \draw[arrow,purple,dashed] (T1) -- (B);
      \draw[arrow] (C) -- (T3);
      \draw[arrow,purple,dashed] (T2) -- (C);
      \draw[arrow,purple,dashed] (T3) to[bend left=40] (A);
...
\begin{frame}[fragile]
  ...
  \begin{block}<2->{\faLightbulb \quad Haskell analogue}
    Think about \texttt{map} and \texttt{fold} or \texttt{foldMap}.
  \end{block}
\end{frame}
...
\begin{frame}[fragile]
  ...
  \begin{itemize}
  \item[\faCode]<1-> Week17/Starvation/TaskQueueDemo.java : example of
    starvation.
  \item[\faCode]<1-> Week17/Livelock/PhilosopherDemo.java : demonstrating a
    livelock using ``polite'' philosophers.
  \item[\faCode]<1-> Week17/Streams/* : examples with parallel Streams.
  \end{itemize}
