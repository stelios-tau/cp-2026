...
\lstset{
  ...
  rulecolor = \color{black}, %% snnnet frame color to avoid being affected by text color
...
\begin{frame}[fragile]
  ...
  \item[\faCode]<1-> LockTestHarness/SyncVSGaren.java: Spinlock vs
    \texttt{synchronized} (single-run)
  \item[\faCode]<1-> LockTestHarness/SyncVSGarenCampaign.java: Spinlock vs
    \texttt{synchronized} (benchmark)
  \item[\faCode]<1-> LatchTestHarness/GlobalVSLocal.java: Latch (local
    updates) vs
    \texttt{synchronized} (single)
  \item[\faCode]<1-> LatchTestHarness/GlobalVSLocalCampaign.java: Latch (local
    updates) vs
    \texttt{synchronized} (benchmark)
  \item[\faCode]<1-> LatchTestHarness/LocalLongCampaign.java: Proper
    benchmarking for latching and local updates using \texttt{long}'s.
  \item[\faCode]<1-> LatchTestHarness/LocalVSMapCampaign.java: Latch
